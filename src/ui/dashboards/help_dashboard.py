...
class HelpPage(QWidget):
    ...
    def _build_overview_html(self) -> str:
        ...
        <p>
            This application calculates and reviews mine water balance using monthly data.
            It combines operating data, environmental conditions, and storage status into one monthly balance.
        </p>
        <h3>How the system runs (high level)</h3>
        <ol>
            <li>Load data sources (Excel meter readings, environmental data, storage facilities).</li>
            <li>Calculate inflows, outflows, storage change, recycled water, KPIs.</li>
            <li>Compute closure error and quality status.</li>
            <li>Store history and sync current storage values.</li>
        </ol>
        <p class="source-ref">Technical reference: <code>services/calculation/balance_service.py</code></p>
        <a name="overview-main-pages"></a>
        <h3>Main pages</h3>
        <ul>
            <li><b>Dashboard:</b> KPI summary cards and system snapshot.</li>
            <li><b>Analytics:</b> Trends from Meter Readings Excel.</li>
            <li><b>Monitoring:</b> Static levels, borehole monitoring, PCD monitoring.</li>
            <li><b>Storage Facilities:</b> Facility records, capacity, utilization and monthly params.</li>
            <li><b>Calculations:</b> Core monthly water balance and quality tabs.</li>
            <li><b>Flow Diagram:</b> Flow map, categorization, recirculation and balance check.</li>
            <li><b>Settings:</b> Constants and environmental monthly datasets.</li>
            <li><b>Messages:</b> Notifications, updates, and feedback workflow.</li>
        </ul>
        <a name="overview-data-stores"></a>
        <h3>Primary data stores</h3>
        <ul>
            <li>Monthly climate records (rainfall and evaporation).</li>
            <li>Storage facility register and month-to-month storage history.</li>
            <li>Flow diagram configuration and flow category setup.</li>
            <li>Meter Readings Excel workbook.</li>
        </ul>
        <p class="source-ref">Technical reference: <code>database/*</code>, <code>services/excel_manager.py</code>, <code>ui/dashboards/*.py</code></p>
        """
    ...
    def _build_dashboards_html(self) -> str:
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/analytics_dashboard.py</code></p>
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/monitoring_dashboard.py</code>, <code>services/directory_loader.py</code></p>
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/storage_facilities_dashboard.py</code></p>
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/calculation_dashboard.py</code></p>
        <a name="dash-flow"></a>
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/flow_diagram_page.py</code></p>
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/settings_dashboard.py</code></p>
        """
    ...
    def _build_calculations_html(self) -> str:
        ...
        <p>
            This page runs the monthly water balance workflow from raw inputs to final closure status.
        </p>
        <a name="calc-order"></a>
        <h3>Execution order</h3>
        <ol>
            <li>Inflows service</li>
            <li>Outflows service</li>
            <li>Storage service</li>
            <li>Recycled water service</li>
            <li>Balance closure + KPI service</li>
            <li>Persist storage history / refresh current storage values</li>
        </ol>
        <p class="source-ref">Technical reference: <code>services/calculation/balance_service.py</code> (<code>BalanceService.calculate</code>)</p>
        <a name="calc-system-tab"></a>
        <h3>System Balance tab</h3>
        <ul>
            <li>Compares total inflows, total outflows, and storage delta.</li>
            <li>Shows closure status and error percent.</li>
        </ul>
        <a name="calc-recycled-tab"></a>
        <h3>Recycled Water tab</h3>
        <ul>
            <li>Uses <code>Total Recycled Water</code> directly if present in Excel.</li>
            <li>Fallback components: TSF return estimate + RWD volume.</li>
        </ul>
        <a name="calc-quality"></a>
        <h3>Data Quality tab</h3>
        <ul>
            <li>Shows missing/estimated/simulated flags collected during calculation.</li>
            <li>High closure error often points to categorization, mapping, or missing data issues.</li>
        </ul>
        <h3>Days of Operation tab</h3>
        <ul>
            <li>Estimates average daily use over the selected month.</li>
            <li>Shows how long current stored water can support operations at that usage rate.</li>
        </ul>
        <a name="calc-days-tab"></a>
        <p class="source-ref">Technical reference: <code>ui/dashboards/calculation_dashboard.py</code>, <code>services/calculation/days_of_operation_service.py</code></p>
        """
    ...
    def _build_formulas_html(self) -> str:
        ...
        <h3>Technical Mapping (optional reference)</h3>
        ...
        <p class="source-ref">Technical reference: <code>services/calculation/balance_service.py</code>, <code>ui/dashboards/flow_diagram_page.py</code></p>
        """
    ...
    def _build_water_sources_html(self) -> str:
        ...
        <ul>
            <li>Monthly rainfall values feed rainfall inflow.</li>
            <li>Monthly evaporation values feed evaporation losses.</li>
        </ul>
        <a name="sources-storage-db"></a>
        <h3>Storage Inputs</h3>
        <ul>
            <li>Facility surface area is used to scale rainfall and evaporation impact.</li>
            <li>Current volume and history support opening/closing storage calculations.</li>
        </ul>
        ...
        <ul>
            ...
            <li>Calculations still require essential Meter Reading fields (or proper mappings).</li>
            <li>Flow Diagram values depend on configured sheet/column mapping for each flow.</li>
        </ul>
        <p class="source-ref">Technical reference: <code>services/calculation/balance_service.py</code> (<code>EXCEL_COLUMNS</code>), <code>services/excel_manager.py</code></p>
        """
    ...
    def _build_storage_html(self) -> str:
        ...
        <ul>
            <li>Opening volume is taken from previous month closing history when available.</li>
            <li>If history is missing, current volume fallback is used and quality may be flagged.</li>
            <li>Calculated closing is distributed per facility proportionally where applicable.</li>
        </ul>
        <a name="storage-persistence"></a>
        <h3>Persistence</h3>
        <ul>
            <li>Each run records a monthly storage snapshot.</li>
            <li>Latest closing volume is pushed to the facility current-volume record.</li>
            <li>Facilities page then displays updated current values.</li>
        </ul>
        <a name="storage-warnings"></a>
        <h3>Warnings and safeguards</h3>
        <ul>
            <li>Negative calculated closing is clamped to zero.</li>
            <li>Capacity exceedance creates warning flags.</li>
        </ul>
        <p class="source-ref">Technical reference: <code>services/calculation/balance_service.py</code> (<code>StorageService</code>)</p>
        """
    ...
    def _build_features_html(self) -> str:
        ...
        <p class="source-ref">Technical reference: <code>ui/dashboards/analytics_dashboard.py</code>, <code>ui/dashboards/monitoring_dashboard.py</code>, <code>ui/dashboards/flow_diagram_page.py</code>, <code>services/calculation/balance_service.py</code></p>
        """
    ...
    def _build_troubleshooting_html(self) -> str:
        ...
        <ul>
            ...
            <li><b>High closure error (&gt;5%):</b> review flow categorization, missing intake/loss values, and climate data for the month.</li>
        ...
        <p class="source-ref">Technical reference: <code>services/calculation/balance_service.py</code>, <code>ui/dashboards/flow_diagram_page.py</code>, <code>ui/dashboards/calculation_dashboard.py</code></p>
        """
