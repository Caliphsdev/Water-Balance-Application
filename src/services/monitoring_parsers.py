...
class MonitoringExcelParser(ABC):
    ...
    def _convert_type(self, value: any, target_type: DataType, date_formats: List[str] = None) -> Tuple[any, Optional[str]]:
        """
...
def get_parser(source_def: DataSourceDefinition) -> MonitoringExcelParser:
    ...
    if source_def.id not in _parser_cache:
        _parser_cache[source_def.id] = ParserFactory.create_parser(source_def, 0.85)
