...
class ReportGenerator:
    ...
    def _aggregate_for_scenario(self, scenario_id: int, start: date, end: date) -> dict:
        """Aggregate range using a specific scenario constants override."""
        from utils.config_manager import config as app_config
        # Preserve current active scenario
        original = app_config.get_active_scenario_id()
        try:
            app_config.set_active_scenario(scenario_id)
            # Rebuild calculator with scenario constants
            calc = WaterBalanceCalculator()
            months = []
            totals = {
                'inflows': 0.0, 'outflows': 0.0, 'net': 0.0,
                'plant_consumption_gross': 0.0,'tsf_return': 0.0,'evaporation': 0.0,'discharge': 0.0,
                'surface_water': 0.0,'groundwater': 0.0,'underground': 0.0,'rainfall': 0.0
            }
            current = date(start.year, start.month, 1)
            last = date(end.year, end.month, 1)
            while current <= last:
                bal = calc.calculate_water_balance(current)
                months.append((current, bal))
                totals['inflows'] += bal['inflows']['total']
                totals['outflows'] += bal['outflows']['total']
                totals['net'] += bal['inflows']['total'] - bal['outflows']['total']
                totals['plant_consumption_gross'] += bal['outflows']['plant_consumption_gross']
                totals['tsf_return'] += bal['outflows']['tsf_return']
                totals['evaporation'] += bal['outflows']['evaporation']
                totals['discharge'] += bal['outflows']['discharge']
                totals['surface_water'] += bal['inflows']['surface_water']
                totals['groundwater'] += bal['inflows']['groundwater']
                totals['underground'] += bal['inflows']['underground']
                totals['rainfall'] += bal['inflows']['rainfall']
                # Increment month
                if current.month == 12:
                    current = date(current.year + 1, 1, 1)
                else:
            ...
            if original:
                app_config.set_active_scenario(original)
            else:
                app_config.clear_active_scenario()

    def generate_scenario_comparison_pdf(self, scenario_a: int, start: date, end: date, scenario_b: Optional[int] = None, filename: Optional[str] = None) -> Path:
        """Generate PDF comparing scenario(s) to base or another scenario."""
        base_data = self.aggregate_range(start, end) if not scenario_b else None
        data_a = self._aggregate_for_scenario(scenario_a, start, end)
        data_b = self._aggregate_for_scenario(scenario_b, start, end) if scenario_b else None
        if not filename:
            label = f"scenario_{scenario_a}_vs_{'base' if not scenario_b else scenario_b}"
            filename = f"scenario_compare_{label}_{start.strftime('%Y%m')}_{end.strftime('%Y%m')}.pdf"
        path = self.output_dir / filename

        from database.db_manager import DatabaseManager
        dbm = DatabaseManager()
        scen_a_meta = dbm.execute_query("SELECT * FROM scenarios WHERE scenario_id = ?", (scenario_a,))
        scen_b_meta = dbm.execute_query("SELECT * FROM scenarios WHERE scenario_id = ?", (scenario_b,)) if scenario_b else []
        diffs_a = dbm.get_scenario_diff(scenario_a)
        diffs_b = dbm.get_scenario_diff(scenario_b) if scenario_b else []

        with PdfPages(path) as pdf:
            # Summary page (landscape)
            fig, ax = plt.subplots(figsize=self.A4_LANDSCAPE)
            ax.axis('off')
            ax.set_title('Scenario Comparison Report', fontsize=16, pad=20)
            ya = 0.92
            ax.text(0.05, ya, f"Range: {start} to {end}", fontsize=11); ya -= 0.04
            a_name = scen_a_meta[0]['name'] if scen_a_meta else f'ID {scenario_a}'
            b_name = scen_b_meta[0]['name'] if scen_b_meta else 'Base Constants'
            ax.text(0.05, ya, f"Scenario A: {a_name}", fontsize=11); ya -= 0.03
            ax.text(0.05, ya, f"Scenario B: {b_name}", fontsize=11); ya -= 0.05

            def lines_for(label, totals):
                return [
                    f"{label} Total Inflows: {totals['inflows']:,.0f} m³",
                    f"{label} Total Outflows: {totals['outflows']:,.0f} m³",
                    f"{label} Net Balance: {totals['net']:,.0f} m³",
                    f"{label} Plant Gross: {totals['plant_consumption_gross']:,.0f} m³",
                    f"{label} TSF Return: {totals['tsf_return']:,.0f} m³",
                    f"{label} Evaporation: {totals['evaporation']:,.0f} m³"
                ]
            for ln in lines_for('Scenario A', data_a['totals']):
                ax.text(0.05, ya, ln, fontsize=9); ya -= 0.025
            if data_b:
                ya -= 0.02
                for ln in lines_for('Scenario B', data_b['totals']):
                    ax.text(0.05, ya, ln, fontsize=9); ya -= 0.025
                # Differences
                ya -= 0.02
                ax.text(0.05, ya, 'Differences (A - B)', fontsize=10, weight='bold'); ya -= 0.03
                diff_totals = ['inflows','outflows','net','plant_consumption_gross','tsf_return','evaporation']
                for k in diff_totals:
                    dv = data_a['totals'][k] - data_b['totals'][k]
                    ax.text(0.05, ya, f"{k}: {dv:,.0f} m³", fontsize=9); ya -= 0.02
            elif base_data:
                ya -= 0.02
                ax.text(0.05, ya, 'Differences (Scenario A - Base)', fontsize=10, weight='bold'); ya -= 0.03
                diff_totals = ['inflows','outflows','net','plant_consumption_gross','tsf_return','evaporation']
                for k in diff_totals:
                    dv = data_a['totals'][k] - base_data['totals'][k]
                    ax.text(0.05, ya, f"{k}: {dv:,.0f} m³", fontsize=9); ya -= 0.02
            pdf.savefig(fig); plt.close(fig)

            # Constants diff page for Scenario A
            figd, axd = plt.subplots(figsize=self.A4_LANDSCAPE)
            axd.axis('off')
            axd.set_title(f'Constants Differences - {a_name}', fontsize=14, pad=15)
            y = 0.94
            for d in diffs_a[:45]:  # limit
                y -= 0.02
                axd.text(0.05, y, f"{d['constant_key']}: base {d['base_value']} → {d['scenario_value']} (Δ {d['delta']:+.3f}{' / '+str(round(d['percent_change'],2))+'%' if d['percent_change'] is not None else ''})", fontsize=8)
                if y < 0.05: break
            if not diffs_a:
                axd.text(0.05, 0.90, 'No differences from base constants.', fontsize=10)
            pdf.savefig(figd); plt.close(figd)

            # Optional second scenario diff page
            if diffs_b:
                figd2, axd2 = plt.subplots(figsize=self.A4_LANDSCAPE)
                axd2.axis('off')
                axd2.set_title(f'Constants Differences - {b_name}', fontsize=14, pad=15)
                y2 = 0.94
                for d in diffs_b[:45]:
                    y2 -= 0.02
                    axd2.text(0.05, y2, f"{d['constant_key']}: base {d['base_value']} → {d['scenario_value']} (Δ {d['delta']:+.3f}{' / '+str(round(d['percent_change'],2))+'%' if d['percent_change'] is not None else ''})", fontsize=8)
                    if y2 < 0.05: break
                pdf.savefig(figd2); plt.close(figd2)

            # Flow diagram for Scenario A (use linear flow for clarity)
            try:
                fig_flow_a = self.create_linear_flow_figure(data_a)
                pdf.savefig(fig_flow_a); plt.close(fig_flow_a)
            except Exception:
                pass
            if data_b:
                try:
                    fig_flow_b = self.create_linear_flow_figure(data_b)
                    pdf.savefig(fig_flow_b); plt.close(fig_flow_b)
                except Exception:
                    pass
        return path
